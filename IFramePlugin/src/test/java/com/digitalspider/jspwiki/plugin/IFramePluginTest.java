...
public class IFramePluginTest extends TestCase {
	...
	public void testUrlValidator() {
		String url = IFramePlugin.findFirstByRegex(VALID_URL, IFramePlugin.REGEX_URL);
		assertEquals(VALID_URL+" should have been a valid URL", VALID_URL, url);
		url = IFramePlugin.findFirstByRegex(FUNNY_URL, IFramePlugin.REGEX_URL);
		assertEquals(FUNNY_URL+" should have been a valid URL", FUNNY_URL, url);
		url = IFramePlugin.findFirstByRegex(FILE_URL, IFramePlugin.REGEX_URL);
		assertEquals(FILE_URL+" should have been a valid URL", FILE_URL, url);
		url = IFramePlugin.findFirstByRegex(INVALID_URL, IFramePlugin.REGEX_URL);
		assertNull(url);
		assertNotSame(INVALID_URL+" should have been an invalid URL", INVALID_URL, url);
	}

	public void testFindImages() {
		Collection<String> images = IFramePlugin.findImages(HTML_DATA);
	...
	public void testPlainTextValidation() {
		String value = IFramePlugin.findFirstByRegex("garfield",IFramePlugin.REGEX_PLAINTEXT);
		assertEquals("garfield", value);
		assertEquals("dilbert-classics", IFramePlugin.findFirstByRegex("dilbert-classics",IFramePlugin.REGEX_PLAINTEXT));
		assertEquals("dil", IFramePlugin.findFirstByRegex("dil//bert-classics",IFramePlugin.REGEX_PLAINTEXT));
		assertEquals("dilbert", IFramePlugin.findFirstByRegex("dilbert?classics",IFramePlugin.REGEX_PLAINTEXT));
		assertEquals("di", IFramePlugin.findFirstByRegex("di&lbert-classics",IFramePlugin.REGEX_PLAINTEXT));
	}
}
